...
void train_nn(const DataSet* train_data, const DataSet* test_data, size_t num_classes, size_t hidden_dim, size_t epochs, float lr, size_t batch)
{
    ...
    size_t size_fc1_temp = test_data->images_num * hidden_dim;
    ...
    for (size_t epoch = 0; epoch < epochs; epoch++)
    {
        ...
        for (size_t offset = 0; offset < test_input_num; offset += batch)
        {
            ...
            // BEGIN YOUR CODE HERE ->
                ...
                // First FC Layer:
                float* fc1_batch = fc1_temp + offset * hidden_dim;
                gemm(array_batch, W1, fc1_batch, m_b, test_data->input_dim, hidden_dim);
                add_bias(fc1_batch, fc1_batch, b1, m_b, hidden_dim);
                Relu(fc1_batch, fc1_batch, m_b * hidden_dim);
                
                // Second FC Layer:
                float* test_result_batch = test_result + offset * num_classes;
                gemm(fc1_batch, W2, test_result_batch, m_b, hidden_dim, num_classes);
                add_bias(test_result_batch, test_result_batch, b2, m_b, num_classes);
